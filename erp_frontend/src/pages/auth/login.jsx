...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    setErrorMessage(''); // Purana error clear karo
    ...
    try {
      ...
      if (response.ok) {
        // Login Success -> Save token and user data in local storage
        localStorage.setItem('userInfo', JSON.stringify(data));
        navigate('/dashboard');
      } else {
        // Login Failed -> Show stylish backend error message
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-950 py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-[#084e8d]/20 rounded-full blur-3xl pointer-events-none"></div>
      <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-[#084e8d]/10 rounded-full blur-3xl pointer-events-none"></div>

      <div className="max-w-md w-full bg-[#111827] p-10 rounded-2xl shadow-[0_0_40px_rgba(0,0,0,0.5)] border border-slate-800 relative z-10">
        ...
        <form className="space-y-6" onSubmit={handleLogin}>
          ...
          {errorMessage && (
            <div className="bg-[#e9272e]/10 border border-[#e9272e]/30 p-3 rounded-lg flex items-start animate-in fade-in zoom-in-95 duration-200">
              ...
              <p className="text-sm text-[#e9272e] font-medium leading-relaxed">
                {errorMessage}
              </p>
            </div>
          )}
          ...
          <div>
            ...
            <input 
              ...
              placeholder="••••••••"
              className="block w-full px-4 py-3 bg-white border border-slate-700 rounded-lg text-sm text-black placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-[#084e8d] focus:border-transparent transition-all shadow-inner" 
            />
          </div>
