...
export default function LeadsSales() {
  ...
  return (
    <div className="w-full px-4 sm:px-6 lg:px-8 py-4 sm:py-6 space-y-4 sm:space-y-6 overflow-x-hidden">
      
      {/* Header Section */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-2xl sm:text-3xl font-extrabold text-[#084e8d] tracking-tight">Student Leads Hub</h2>
          <p className="text-xs sm:text-sm text-slate-500 mt-1">Click on the chart below to filter your pipeline data.</p>
        </div>
        <button 
          onClick={() => setIsAddModalOpen(true)}
          className="w-full md:w-auto flex items-center justify-center bg-[#084e8d] text-white px-5 py-2.5 rounded-lg text-sm font-bold hover:bg-[#063a6b] shadow-lg shadow-[#084e8d]/20 transition-all transform hover:-translate-y-0.5"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 sm:gap-6">
        ...
        <div className="lg:col-span-1 bg-gradient-to-br from-[#084e8d] to-[#063a6b] rounded-2xl p-5 sm:p-6 text-white shadow-sm flex flex-col justify-center relative overflow-hidden">
          <Activity size={100} className="absolute -right-6 -bottom-6 text-white/10" />
          <h3 className="text-blue-100 text-xs sm:text-sm font-bold uppercase tracking-wider mb-2 relative z-10">Total Active Pipeline</h3>
          <div className="text-5xl sm:text-6xl font-black mb-2 sm:mb-4 relative z-10">{totalLeads}</div>
          <p className="text-[11px] sm:text-xs font-medium text-blue-200 relative z-10">Total student inquiries currently in the system.</p>
        </div>

        {/* Visual Interactive Bar Chart */}
        <div className="lg:col-span-3 bg-white p-4 sm:p-6 rounded-2xl shadow-sm border border-slate-200 overflow-hidden w-full">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 sm:mb-6 gap-3">
            <h3 className="text-xs sm:text-sm font-bold text-slate-500 uppercase tracking-wider flex items-center">
              <BarChart2 size={16} className="mr-2 text-[#084e8d]" /> Pipeline Status Chart
            </h3>
            {statusFilter !== 'All' && (
              <button 
                onClick={() => setStatusFilter('All')} 
                className="text-[11px] sm:text-xs font-bold text-[#e9272e] bg-red-50 hover:bg-red-100 px-3 py-1.5 rounded-md transition-colors"
              >
                Clear Filter
              </button>
            )}
          ...
          {/* Added horizontal scroll for small devices to prevent squishing bars */}
          <div className="overflow-x-auto no-scrollbar w-full pb-2">
            <div className="flex items-end h-32 gap-2 sm:gap-4 justify-between sm:justify-around px-1 min-w-[500px] sm:min-w-0">
              {graphData.map((stat) => {
                ...
                return (
                  <div 
                    ...
                    className={`flex flex-col items-center justify-end w-full cursor-pointer group transition-all flex-1`}
                    title={`Click to view ${stat.name} students`}
                  >
                    <span className={`text-[10px] sm:text-xs font-black mb-2 transition-colors ${isSelected ? 'text-slate-900 scale-110' : 'text-slate-400 group-hover:text-slate-600'}`}>
                      {stat.count}
                    </span>
                    
                    <div className="w-full relative flex justify-center h-full">
                      <div 
                        className={`w-full max-w-[30px] sm:max-w-[40px] rounded-t-lg transition-all duration-500 flex justify-center items-end pb-2 ${stat.color} 
                    ...
                    <span className={`text-[9px] sm:text-[11px] font-extrabold mt-2 sm:mt-3 text-center uppercase tracking-wider transition-colors truncate w-full px-0.5 ${isSelected ? 'text-slate-800' : 'text-slate-400 group-hover:text-slate-600'}`} title={stat.name}>
                      {stat.name}
                    </span>
                  </div>
                );
      ...
      <div className="flex flex-col bg-white rounded-2xl shadow-sm border border-slate-200">
        ...
        <div className="bg-slate-50 border-b border-slate-200 p-4 sm:p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 z-20 rounded-t-2xl shadow-sm">
          ...
          <div className="flex items-center justify-end w-full md:w-auto">
            {statusFilter !== 'All' ? (
              <span className="text-[11px] sm:text-sm font-bold text-slate-600 flex items-center bg-white px-3 sm:px-4 py-2 border border-slate-200 rounded-lg shadow-sm w-full sm:w-auto justify-between sm:justify-start">
                Showing: 
                <span className={`ml-2 px-2 py-0.5 rounded text-[10px] sm:text-xs uppercase tracking-wider whitespace-nowrap
              ...
              <span className="text-xs sm:text-sm font-bold text-slate-500">Showing All Pipeline Data</span>
            )}
          </div>
        </div>

        {/* INNER TABLE AREA - Overflow added here */}
        <div className="w-full bg-slate-50/30 overflow-x-auto custom-scrollbar">
          <table className="min-w-full divide-y divide-slate-200">
            <thead className="bg-[#084e8d] text-white">
              <tr>
                <th className="px-4 sm:px-6 py-3 sm:py-4 text-left text-[11px] sm:text-xs font-bold uppercase tracking-wider whitespace-nowrap">Student Info</th>
                <th className="px-4 sm:px-6 py-3 sm:py-4 text-left text-[11px] sm:text-xs font-bold uppercase tracking-wider whitespace-nowrap">Contact Details</th>
                <th className="px-4 sm:px-6 py-3 sm:py-4 text-left text-[11px] sm:text-xs font-bold uppercase tracking-wider whitespace-nowrap">Hierarchy / Assignee</th>
                <th className="px-4 sm:px-6 py-3 sm:py-4 text-left text-[11px] sm:text-xs font-bold uppercase tracking-wider w-64 sm:w-72 whitespace-nowrap">Status & Manual Remark</th>
              ...
              {paginatedLeads.length > 0 ? (
                    ...
                    <tr key={lead.id} className="hover:bg-slate-50 transition-colors">
                      <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                        <div className="flex flex-col">
                          ...
                          <span className="text-[11px] sm:text-xs font-extrabold text-blue-600 mt-0.5">{lead.id}</span>
                      ...
                      <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                        <div className="flex flex-col space-y-1">
                          ...
                          <div className="flex items-center text-[11px] sm:text-xs text-slate-500">
                      ...
                      <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                        <div className="flex flex-col border-l-2 border-[#084e8d]/20 pl-3">
                          ...
                          <span className="text-xs sm:text-sm font-bold text-[#084e8d] mb-1">{lead.manager}</span>
                          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">BDE</span>
                          <span className="text-xs sm:text-sm font-medium text-slate-700">{lead.bde}</span>
                        </div>
                      </td>
                      <td className="px-4 sm:px-6 py-4 min-w-[250px]">
                        <div className="flex flex-col">
                          <span className={`px-2.5 py-1 inline-flex text-[10px] sm:text-[11px] leading-5 font-extrabold uppercase tracking-widest rounded-md w-fit border ${leadConfig?.bg} ${leadConfig?.text} border-${leadConfig?.color.split('-')[1]}-200`}>
                            {lead.status}
                          </span>
                          <div className="mt-2 bg-slate-50 p-2.5 rounded border border-slate-100">
                            <span className="block text-[11px] font-medium text-slate-600 italic line-clamp-2 whitespace-normal" title={lead.remarkText}>
                      ...
                      <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-right text-sm font-medium align-top">
                        <button 
                          ...
                          className="flex items-center justify-center w-full sm:w-auto text-[#084e8d] bg-white border border-slate-200 hover:bg-[#084e8d]/5 hover:border-[#084e8d]/30 px-3 py-2 rounded-lg shadow-sm transition-colors font-bold text-xs" 
                          title="Update Lead"
                        >
                          <Edit2 size={14} className="mr-1.5 flex-shrink-0" /> Edit & Update
                        </button>
                      </td>
                    </tr>
                  )
                ...
                <tr>
                  <td colSpan="5" className="px-6 py-12 text-center text-slate-500">
                    ...
                    <p className="text-xs sm:text-sm mt-1">Try selecting a different bar on the chart or clear search.</p>
                  </td>
                </tr>
              )}
            </tbody>
        ...
        {/* PAGINATION FOOTER */}
        <div className="bg-white border-t border-slate-200 px-4 sm:px-6 py-4 flex flex-col sm:flex-row items-center justify-between gap-4 rounded-b-2xl shadow-[0_-10px_15px_-3px_rgba(0,0,0,0.05)]">
          ...
          <div className="flex items-center space-x-2 w-full sm:w-auto justify-center">
            ...
            <span className="text-[11px] sm:text-sm font-bold text-[#084e8d] px-3 sm:px-4 py-1.5 bg-blue-50 border border-blue-100 rounded-md whitespace-nowrap">
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl overflow-hidden animate-in fade-in zoom-in duration-200 flex flex-col max-h-[90vh]">
            <div className="bg-[#084e8d] px-4 sm:px-6 py-4 flex justify-between items-center shrink-0">
              <h3 className="text-base sm:text-lg font-bold text-white flex items-center">
                <UserPlus size={18} className="mr-2 flex-shrink-0" /> Add New Student into System
              </h3>
              <button onClick={() => setIsAddModalOpen(false)} className="text-blue-200 hover:text-white transition-colors flex-shrink-0">
                <X size={20} className="sm:w-6 sm:h-6" />
              </button>
            </div>
            
            <div className="overflow-y-auto custom-scrollbar p-4 sm:p-6 flex-1">
              <form onSubmit={handleAddLead}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-x-8 sm:gap-y-6">
                  
                  <div className="space-y-4">
                    <h4 className="text-[11px] sm:text-xs font-extrabold text-slate-400 uppercase tracking-widest border-b pb-2">Student Information</h4>
                  ...
                  <div className="space-y-4">
                    <h4 className="text-[11px] sm:text-xs font-extrabold text-slate-400 uppercase tracking-widest border-b pb-2">Hierarchy Setup</h4>
                  ...
                  <div className="md:col-span-2 space-y-4 pt-2">
                    <h4 className="text-[11px] sm:text-xs font-extrabold text-slate-400 uppercase tracking-widest border-b pb-2">Pipeline Status & Remarks</h4>
                ...
                <div className="mt-6 sm:mt-8 pt-4 sm:pt-5 border-t border-slate-100 flex flex-col sm:flex-row justify-end gap-3">
                  <button type="button" onClick={() => setIsAddModalOpen(false)} className="w-full sm:w-auto px-5 py-2.5 text-sm font-bold text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-lg transition-colors">
                    Cancel
                  </button>
                  <button type="submit" className="w-full sm:w-auto px-5 py-2.5 text-sm font-bold text-white bg-[#084e8d] hover:bg-[#063a6b] rounded-lg shadow-md shadow-[#084e8d]/30 transition-all">
                    Save Student Data
                  </button>
                </div>
              </form>
      ...
      {isEditModalOpen && editLeadData && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden animate-in fade-in zoom-in duration-200 flex flex-col max-h-[90vh]">
            <div className="bg-[#084e8d] px-4 sm:px-6 py-4 flex justify-between items-center shrink-0">
              <h3 className="text-base sm:text-lg font-bold text-white flex items-center">
                <Edit2 size={18} className="mr-2 flex-shrink-0" /> Update Lead Status
              </h3>
              <button onClick={() => setIsEditModalOpen(false)} className="text-blue-200 hover:text-white transition-colors flex-shrink-0">
                <X size={20} className="sm:w-6 sm:h-6" />
              </button>
            </div>
            ...
            <div className="overflow-y-auto custom-scrollbar p-4 sm:p-6 flex-1">
              <form onSubmit={handleUpdateLead}>
                <div className="mb-4 sm:mb-6 pb-4 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                  <div>
                    <h4 className="text-sm font-bold text-slate-800">{editLeadData.name}</h4>
                    <p className="text-[11px] sm:text-xs text-slate-500 mt-0.5">{editLeadData.id} • {editLeadData.phone}</p>
                  </div>
                  <div className="sm:text-right bg-slate-50 sm:bg-transparent p-2 sm:p-0 rounded-lg w-full sm:w-auto">
                ...
                <div className="space-y-4 sm:space-y-5">
                  <div>
                    ...
                    <select required value={editLeadData.status} onChange={e => setEditLeadData({...editLeadData, status: e.target.value})} className="w-full px-3 py-2.5 sm:py-3 border border-slate-300 rounded-lg text-sm font-bold text-[#084e8d] focus:outline-none focus:ring-2 focus:ring-[#084e8d] focus:border-transparent bg-blue-50/50">
                      {pipelineStatuses.map(stat => (
                        <option key={stat.name} value={stat.name}>{stat.name}</option>
                      ))}
                    </select>
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1 flex items-center"><Edit3 size={14} className="mr-1 text-[#084e8d]"/> Manual Note (Client's Exact Words) *</label>
                    <textarea required rows="4" value={editLeadData.remarkText} onChange={e => setEditLeadData({...editLeadData, remarkText: e.target.value})} className="w-full px-3 sm:px-4 py-2.5 sm:py-3 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#084e8d] focus:border-transparent resize-none" placeholder="Type the latest discussion details here..."></textarea>
                  </div>
                </div>

                <div className="mt-6 sm:mt-8 flex flex-col sm:flex-row justify-end gap-3">
                  <button type="button" onClick={() => setIsEditModalOpen(false)} className="w-full sm:w-auto px-5 py-2.5 text-sm font-bold text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-lg transition-colors">
                    Cancel
                  </button>
                  <button type="submit" className="w-full sm:w-auto px-5 py-2.5 text-sm font-bold text-white bg-[#084e8d] hover:bg-[#063a6b] rounded-lg shadow-md shadow-[#084e8d]/30 transition-all">
